...
import { getUnlockedRoomOrder, isRoomUnlocked } from '../../utils/progress';
...
export default function SpaceMap() {
  ...
  const [phase, setPhase] = useState<SpacePhase>('preview');
  ...
  useEffect(() => {
    const sync = () => setUnlockedOrder(getUnlockedRoomOrder());
    window.addEventListener('mecfs-progress', sync);
    return () => window.removeEventListener('mecfs-progress', sync);
  }, []);

  const scale = phase === 'preview' ? 0.6 : 1;
  const prompt =
    phase === 'preview'
      ? 'Click the space'
      : unlockedOrder < 4
        ? `Stage ${unlockedOrder} available · complete in order`
        : 'Explore rooms · hover to enter';
  ...
  const handleEnterRoom = (room: RoomConfig) => {
    ...
    setTimeout(() => navigate(room.path), 450);
  };
  ...
  return (
    <section className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden">
      ...
      >
        <motion.p
          className="absolute top-10 md:top-14 left-0 right-0 text-center text-[10px] md:text-xs font-mono tracking-[0.35em] text-slate-500 uppercase z-20 pointer-events-none"
          animate={{ opacity: phase === 'explore' ? 0.55 : 0.85 }}
        >
          {prompt}
        </motion.p>
        ...
        <div className="relative w-full max-w-5xl flex items-center justify-center py-16 md:py-20">
          ...
          >
            <Pseudo3DImage scale={scale} enabled={!isEnteringRoom}>
              <motion.button
                type="button"
                className={`relative block w-full max-w-[min(920px,92vw)] mx-auto rounded-sm overflow-visible ${
              ...
              >
                <div className="relative w-full h-full pseudo-3d-image-wrap">
                  <img
                    src={ASSETS.spaceMap}
                    ...
                    draggable={false}
                  />
                  <div className="absolute inset-0 pointer-events-none shadow-[0_32px_100px_rgba(0,0,0,0.9),0_0_48px_rgba(255,255,255,0.03)_inset]" />
                </div>
        ...
        {phase === 'preview' && (
          <motion.p
            className="absolute bottom-12 md:bottom-16 text-[10px] font-mono tracking-[0.3em] text-slate-600 uppercase z-20"
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.7 }}
